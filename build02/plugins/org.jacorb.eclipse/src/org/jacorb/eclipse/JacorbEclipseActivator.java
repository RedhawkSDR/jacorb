...
public class JacorbEclipseActivator extends Plugin {
	...
	public void start(BundleContext bundleContext) throws Exception {
		...
		configureJavaLogger(bundleContext);
		configureJacorb(bundleContext);
	}
	...
	private void configureJacorb(BundleContext context) {
		...
		try {
			...
			getLog().log(new Status(Status.WARNING, PLUGIN_ID, "Failed to find configure/jacorb.config.dir, using default configuration: " + propUrl, e));
		} finally {
			if (propUrl != null) {
				URL fileUrl;
				try {
					...
					getLog().log(new Status(Status.WARNING, PLUGIN_ID, "Failed to configure jacorb.config.dir location.  CORBA operations may not work.", e));
				} catch (URISyntaxException e) {
					getLog().log(new Status(Status.WARNING, PLUGIN_ID, "Failed to configure jacorb.config.dir location.  CORBA operations may not work.", e));
				}
			} else {
				getLog().log(new Status(Status.WARNING, PLUGIN_ID, "Failed to configure jacorb.config.dir location.  CORBA operations may not work.", null));
			}
		}
	}
